...
def half_incidence(graph, ordering='blocks', return_ordering=False):
    ...
    def make_coo(make_pairs):
        ...
        return scipy.sparse.coo_matrix((data, list(zip(*coords))),
...
def nb_matrix(graph, aux=False, ordering='blocks', return_ordering=False):
    ...
    if aux:
        ...
        degrees = scipy.sparse.diags([degrees[n] for n in graph.nodes()])
        ident = scipy.sparse.eye(graph.order())
        adj = nx.adjacency_matrix(graph)
        pseudo = scipy.sparse.bmat([[None, degrees - ident], [-ident, adj]])
        ...
        nbm = scipy.sparse.coo_matrix((data, list(zip(*h_coords))),
...
def perm_matrix(size):
    ...
    """
    return scipy.sparse.bmat([[None, scipy.sparse.identity(size)],
                        [scipy.sparse.identity(size), None]])
...
def x_matrix(graph, remove_node=None, add_neighbors=None, return_all=False):
    ...
    if remove_node is not None:
        ...
        if degree == 0:
            B = nb_matrix(graph)
            D = scipy.sparse.csr_matrix(np.empty((0, 0)))
            E = scipy.sparse.csr_matrix(np.empty((0, 0)))
            F = scipy.sparse.csr_matrix(np.empty((0, 0)))
            return (B, D, E, F) if return_all else D.dot(F.dot(E))
...
def x_nb_centrality(graph, approx=True, return_eigenvalue=False, tol=0):
    ...
    if approx:
        ...
        for node in graph:
            ...
            X = D.dot(F).dot(E)
            val, right = scipy.sparse.linalg.eigs(B, k=1, tol=tol)
